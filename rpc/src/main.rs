...
fn main() {
    ...
    println!("rect physics half extents: {:?}", rect.get_half_extents_for_physics());
    println!(
    ...
    let input = MoveInputState { up: false, down: false, left: false, right: true };
    for i in 0..100 {
        logic.update_position_with_input(PlayerSide::Left, &input);
        logic.step_physics();
        println!("tick {i}: left pos = {:?}", logic.get_position(PlayerSide::Left));
    }
